...
document.addEventListener('click', function(e){
    if(memeModal.style.display === 'flex' && !memeModal.contains(e.target) && e.target !== getImageBtn){
        closeModal()
    }
})
...
function renderCat(){
    ...
    if(!catObject){
        // Provide visual indication to select an emotion if none is chosen
        emotionRadios.classList.add('shake')
        setTimeout(() => {
            emotionRadios.classList.remove('shake')
        }, 400)
        return
    }
